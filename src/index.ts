...
export class Changelly {
    ...
    private _sign(message: object): string {
        // @ts-ignore
        return crypto
            .sign("sha256", Buffer.from(JSON.stringify(message)), {
                // @ts-ignore
                key: this.privKey,
                type: "pkcs8",
                format: "der",
    ...
    private async _request(
        ...
        return new Promise((resolve, reject) => {
            axios
                .post(Changelly.url, message, axiosConfig)
    ...
    public async getPairsParams(
        params: CurrencyPair[],
    ): Promise<ChangellyResponse<CurrencyPairInfo[]>> {
        if (params.length === 0) {
    ...
    public async getStatus(
        id: string,
    ): Promise<ChangellyResponse<TransactionStatus>> {
        const params = {
            id: id,
        };
    ...
    public async getFixRate(
        params: CurrencyPair[],
    ): Promise<ChangellyResponse<ExchangeInfo[]>> {
        if (params.length === 0) {
            throw new Error("Pairs empty");
        }
    ...
    public async getFixRateForAmount(
        params: CurrencyPairWithAmount[],
    ): Promise<ChangellyResponse<ExchangeInfoWithAmount[]>> {
        if (params.length === 0) {
            throw new Error("Pairs empty");
        }
    ...
    public async validateAddress(
        currency: string,
        address: string,
    ): Promise<ChangellyResponse<ValidationResult>> {
        const params = {
            currency: currency,
            address: address,
        };
